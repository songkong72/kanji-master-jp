...
data.forEach(d => {
    const processSet = (type) => {
        ...
        readings.forEach((rawR, idx) => {
            const fullR = kataToHira(rawR).replace('.', ''); // Search by full reading (non-ruby)
            const kanjiR = kataToHira(rawR).split('.')[0];

            let foundIdx = -1;
            // Best match: exact reading string in the ruby part
            foundIdx = pool.findIndex((s, i) => !usedIndices.has(i) && s.text.includes(`(${fullR})`));
            if (foundIdx === -1) {
                foundIdx = pool.findIndex((s, i) => !usedIndices.has(i) && s.text.includes(`(${kanjiR})`));
            }
            ...
            if (foundIdx !== -1) {
                ...
                const matchingEx = d.examples.find(ex => {
                    ...
                    return exR === fullR || exR === kanjiR || exR.includes(fullR);
                });
                ...
                if (matchingEx) {
                    ...
                    finalSents.push({
                        text: `[${d.kanji}](${kanjiR})${rawR.includes('.') ? rawR.split('.')[1] : ''}${t.jp}`,
...
console.log('Fixed Parity Logic: Full reading search enabled.');
